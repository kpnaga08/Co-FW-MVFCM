function [Acc,rand_index,match,JI]=AccMeasure(T,idx)
...
for xi=1:n-1
    for xj=xi+1:n
        ...
        rand_sd1=rand_sd1+((idx(xi)==idx(xj))&&(T(xi)~=T(xj)));
        rand_ds1=rand_ds1+((idx(xi)~=idx(xj))&&(T(xi)==T(xj)));
        rand_dd1=rand_dd1+((idx(xi)~=idx(xj))&&(T(xi)~=T(xj)));
    end
end
rand_index=2*(rand_ss1+rand_dd1)/(n*(n-1));
Acc=Acc1/n; 
match=[1:k;match];
JI=rand_ss1/(rand_ss1+rand_sd1+rand_ds1);